...
/***/ function(module, exports, __webpack_require__) {
	...
	PageController = (function() {
	  ...
	  JournalTotalsCtrl = __webpack_require__(14);
	
	  function PageController(journalPreparedFp) {
	    var State, defaultState, editJournalController, editState, journalTotalsController, totalsState;
	    this.journalCtrl = new JournalCtrl(journalPreparedFp);
...
/***/ function(module, exports, __webpack_require__) {
	...
	JournalCtrl = (function() {
	  var QuickEditController, _hideInfoMessage, _showInfoMessage, layoutManagerCtr, renderCtr, renderJournal;
	
	  renderCtr = __webpack_require__(4);
	
	  layoutManagerCtr = __webpack_require__(6);
	
	  QuickEditController = __webpack_require__(7);
	
	  function JournalCtrl(preparedFp) {
	    var controlButtons, fpContainer, fpModel, fpSources;
	    ...
	    this.filterPanel = new filterPanel(fpContainer, fpModel, fpSources, "/webapi/grade/journal/filter/init", null, null, true, true);
	  ...
	  renderJournal = function(model, ctrl) {
	    ...
	    if (!journalModel.editLimit.readOnly && journalModel.markSettings.maxMark === 5) {
	  ...
	  JournalCtrl.prototype.editAssignments = function() {
	    return postTo("EditJournalAssignments.asp", {
	      BACK: "Journal.asp"
	    });
	  };
	  ...
	  JournalCtrl.prototype.editTotals = function() {
	    ...
	    return postTo("EditTotal.asp", data);
	  };
	  ...
	  JournalCtrl.prototype.loadJournal = function() {
	    var classId, classIdIup, grade, isIup, rawWal, values;
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
		var JournalRender = function JournalRender() {
			var getAvgMarks = __webpack_require__(5);
			...
			this.GetStudentAvgWithTotalMarks = function (journalData) {
				var avgMarks = _.indexBy(getAvgMarks(journalData.marks, journalData.markSettings.useWeight, journalData.markSettings.minMark, journalData.classMeeting, appContext.isTkr, journalData.studentId), "studentId");
				var totalMarks = {};
				...
				for (var i = 0; i < students.length; i++) {
					...
					var avgMark = avgMarks[_student.id] || {};
					var totalMark = totalMarks[_student.id] || {};
	
					var rowClass = getStudentRowClass(journalData, _student);
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
		var getAvgMarks = function getAvgMarks(marks, useWeight, minMark, classMeetings, isTkr) {
	...
	JournalLayoutManager = (function() {
	  ...
	  JournalLayoutManager.prototype.show = function() {
	    ...
	    setJournalVisible = function() {
	      ...
	      return window.setTimeout(adaptJournalWidth, 100);
	    };
	    return deferredResLoader.ready(function() {
	      return window.setTimeout(setJournalVisible, 250);
	    });
	  };
...
/***/ function(module, exports, __webpack_require__) {
	...
	QuickEditController = (function() {
	  ...
	  gradeConstants = __webpack_require__(9);
	  ...
	  QuickEditController.prototype.init = function() {
	    var buttonClass, ctrl, i, mark, popupButtonsHtml, ref, ref1;
	    ctrl = this;
	    window.dataWereChanged = false;
	    ...
	    return resourceLoader.getAssignTypes().then((function(_this) {
	      return function(assignmentTypes) {
	        var item, j, lastSelected, len, ref2, sortedTypes;
	        ...
	        for (j = 0, len = sortedTypes.length; j < len; j++) {
	          item = sortedTypes[j];
	          $("<option></option>").val(item.id).append(item.name).appendTo(_this.assignTypeControl);
	        }
	        lastSelected = (ref2 = window.localStorage) != null ? ref2.getItem("quick-edit-ass-type") : void 0;
	        if (lastSelected) {
	  ...
	  QuickEditController.prototype.initMarksInput = function() {
	    ...
	    $("<option></option>").val(-100).append("Ручной ввод").appendTo(this.markControl);
	    for (mark = i = ref = this.journalModel.markSettings.maxMark, ref1 = this.journalModel.markSettings.minMark; ref <= ref1 ? i <= ref1 : i >= ref1; mark = ref <= ref1 ? ++i : --i) {
	      $("<option></option>").val(mark).append(mark).appendTo(this.markControl);
	    }
	    $("<option></option>").val(0).append("Точка").appendTo(this.markControl);
	    $("<option></option>").val(-1).append("Очистить").appendTo(this.markControl);
	    ...
	    this.markBtnGroup.append($('<button type="button" class="btn btn-default" data-mark="-100">Ручной ввод</button>').click(quickMarkBtnHandler));
	    ...
	    this.markBtnGroup.append($('<button type="button" class="btn btn-default" data-mark="0">Точка</button>').click(quickMarkBtnHandler));
	    this.markBtnGroup.append($('<button type="button" class="btn btn-default" data-mark="-1">Очистить</button>').click(quickMarkBtnHandler));
	  ...
	  QuickEditController.prototype.setMark = function(mark) {
	    ...
	    markView = mark;
	    if (!this.quickEditExistsAssign && this.quickEditAssignTypeId === gradeConstants.assignmentTypes.homeWork) {
	    ...
	    filterFunc = function(res) {
	      ...
	      return res.assignId === result.assignId || res.assignTypeId === result.assignTypeId;
	    };
...
/***/ function(module, exports, __webpack_require__) {
	...
	JournalTotalsCtrl = (function() {
	  ...
	  templateManager = new (__webpack_require__(15));
...
/***/ }
/******/ ]);
//# sourceMappingURL=journal-bundle.js.map