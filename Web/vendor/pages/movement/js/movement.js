...
arrTempEOTypeReasons[0] = [[10,15,-4,19,25,34],[8,9,10,12,13,14,15,16,18,19,25,-4],[-3],[8,9,10,12,13,14,15,16,18,19,25,-4]];
...
var arrTempEOTypeReasons = new Array();
arrTempEOTypeReasons[-1] = [[2,3],[5,33],[-3],[33]];
arrTempEOTypeReasons[0] = [[2,3],[12,13,16,33],[-3],[12,13,16,33]];
...
//  -3 - все причины
//  -4 - все без itemorder
var moeEoReady = $.Deferred();
var arrMOVEOSes = [];

var initMoveEos = (function() {
	var deferred = moeEoReady;
	moeEoReady = deferred.promise();

	return function (schoolId, funcType, docType, step) {
		jsSubmit({
				action: "/asp/ajax/GetMoveEOs.asp",
				data: { schoolId: schoolId, funcType: funcType, docType: docType, step: step },
				method: "GET"
			})
			.then(function (response) {
				arrMOVEOSes = response.data.list;
				if (arrMOVEOSes) {
					arrMOVEOSes.unshift([-1, language.Generic.Movement.kNotSelected, 0, 0, 0, -1]);
				}
				deferred.resolve(response.data.list);
			});

		moeEoReady.then(function () {
			$("input[type=hidden][id*=EOS_]").each(function (idx, input) {
				var span = $(input).siblings("span[id*=EOS_SPAN]").eq(0);
				var eoId = $(input).val();

				if (eoId > 0) {
					var eo = _.find(arrMOVEOSes, function (moveEoInfo) { return moveEoInfo[0] == eoId; });
					if (eo) {
						var eoName = eo[1];
						span.html(eoName);
					}
...
var reasonValidator = (function(){
	...
	return {
		...
		validateByEOId : function ( reasonIO, EOId ) {
			var moveo = _.find(arrMOVEOSes, function (a) { return a[0] == EOId; });
			if (!moveo) {
				return true;
			}
			var eotypeid = moveo[3];
...
var ctrl_constructor = function(studentIdArg, bwithadding){
	...
	var _reason = (function( reasonlist ){
		...
		var __initItemOrderSet = function()	{
			var itemOrderSet;
			var eoTypeId = _eo.eotypeid() || 0;
			if ( arrEOTypeReasons[nDocType][eoTypeId] && arrEOTypeReasons[nDocType][eoTypeId][nFuncTypeArrayIndex] )
				if( !$.isArray( arrEOTypeReasons[nDocType][eoTypeId][nFuncTypeArrayIndex] ) )
					itemOrderSet = $.makeArray(arrEOTypeReasons[nDocType][eoTypeId][nFuncTypeArrayIndex]);	
				else
					itemOrderSet = arrEOTypeReasons[nDocType][eoTypeId][nFuncTypeArrayIndex];	
			else
