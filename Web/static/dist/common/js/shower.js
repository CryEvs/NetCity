  ...
  $.show = {
    ...
    dialog: function(options) {
      ...
      if (options.buttons) {
        ...
        for (i = 0, len1 = ref1.length; i < len1; i++) {
          ...
          switch (button.label) {
            case language.Generic.Common.kOk:
              ...
              break;
            case language.Generic.Common.kYes:
              setDefButtonStyle(button, "glyphicon glyphicon-ok-sign", "btn-primary");
              break;
            case language.Generic.Common.kNo:
              setDefButtonStyle(button, "glyphicon glyphicon-remove-sign", "btn-default");
              break;
        ...
        if (!existsCancel && !options.withoutCancelButton) {
    ...
    fileDialog: function(options) {
       ...
       */
      var _check, _createHiddenField, _datasubmit, applyBtn, cancelBtn, content, deferred, ref;
      deferred = $.Deferred();
      ...
      content = content.replace('{2}', typeof options.formAppendContent !== 'undefined' ? options.formAppendContent : "");
      $.show.dialog({
        title: options.title,
        message: content,
        onshown: function(dialog) {
          ...
          if (options.isAjax) {
            $('#fileupload').bind('fileuploadsubmit', function(e, data) {
              var _params, formParam, i, len1, ref, ref1;
              _params = {
              ...
              ref = $('form[name=selectFile]').find("input,select").serializeArray();
              for (i = 0, len1 = ref.length; i < len1; i++) {
                formParam = ref[i];
                if (!((ref1 = options.submitParams) != null ? ref1[formParam.name] : void 0)) {
        ...
        buttons: [
          {
            label: (ref = options.applyBtnText) != null ? ref : language.Generic.Common.kOk,
            action: applyBtn
          }, {
            label: language.Generic.Buttons.kCancel,
            action: cancelBtn
    ...
    longWork: function(message, title, settings, getProgress) {
      var content, inc, options, progressBar, setProgress, timer;
      settings = settings || {};
      if (typeof message === 'undefined') {
