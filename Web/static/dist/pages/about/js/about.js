...
$(document).ready(function() {
  var _authContextStore, authContextReady, handleError, idpBindUser, loadTpl, loginState, mobileAccessToken, mobileEntry, templates;
  ...
  idpBindUser = urlHelper.getParameterByName("idpBindUser") !== null;
  _authContextStore = function() {
    var formCollection;
    if (loginState) {
      return jsSubmit({
        action: '/webapi/auth/mobile/info?loginState=' + loginState,
        method: 'GET',
        showProcessing: false,
        defaultErrorHandling: false,
        auth: false
      }).then(function(info) {
        return mobileEntry = info != null ? info.isMobile : void 0;
      }).fail(function(err) {
    ...
    if (mobileEntry) {
      ...
      if (mobileAccessToken) {
        formCollection.mobileAccessToken = mobileAccessToken;
      }
      return jsSubmit({
        action: '/webapi/auth/login-state',
        method: 'POST',
        data: formCollection,
        showProcessing: false,
        defaultErrorHandling: false,
  ...
  authContextReady = _authContextStore();
  return $.when(jsSubmit({
    ...
    return $.when(authContextReady).then(function() {
      var model, netschool;
      model = {
      ...
      if (mobileEntry) {
        ...
        if (model.esia) {
          ...
          if (loginState) {
            model.esia.loginPage += "&loginState=" + loginState;
          }
        }
      }
