...
Dim bAvailavableEOPresent, bCreateEOError, objRS, strEOLegalFormName83, strFoundersHtml
...
Sub Main()
	...
	' Local deployment patch: ICOMDataAccess.GetEOFounders was removed in 5.14; GetStateEOFounders returns all founders
	' of the region, EOID is filled only for founders linked to this organization.
	Set objFounders			= objNSNET.GetStateEOFounders(GetSafeLng(obTokenMgr.GetData(strToken,"State"), 0), GetSafeLng(strEOID, 0))
	strFoundersHtml			= ""
	While Not objFounders.EOF
		If Not IsDull(objFounders("EOID")) Then strFoundersHtml = strFoundersHtml & DB2HTML(objFounders("FNAME")) & "<br />"
		objFounders.MoveNext
	Wend
	bEmptyFounders			= (strFoundersHtml = "")
