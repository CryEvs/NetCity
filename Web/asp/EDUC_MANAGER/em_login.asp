...
<body onload="window_onLoad();"<%If lngLoginState = lngLoginStateLast Then%> onkeypress="CheckEnter(event);"<%End If%> bgcolor="<%=Application("BASECOLOR")(FuncType_School)%>" link="#3341AB" vlink="#6785CD" bgproperties="fixed">
<div align="center">
	<script><!--
...
End If
%></div>
	<table border="0" width="703" height="345" cellpadding="0" cellspacing="0">
	<tr>
		<td width="178" height="89"><img border="0" src="<%=strCommonImgFolder%>/logo_1n.gif" width="178" height="89"></td>
		<td width="483" height="89" align="right"><font color="white"><%=obLanguage("Login","kVersion") & " " & NETSCHOOL_VERSION & "." & NETSCHOOL_REVISION%></font></td>
...
<table border="0" cellpadding="2" cellspacing="0">
	<tr>
	<td colspan="3"><p><%=obLanguage("Login","kSelectDataFromListboxes")%></p></td></tr>
	<tr>
	<td valign="baseline" width="30%"></td>
	<td colspan="2" valign="baseline"><input type="hidden" name="CID" value="<%= DB2Value(strCountryID) %>">&nbsp;</td>
	</tr><tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("Login","kLoginRegion")%>:</td>
	...
	ElseIf nStateID>0 Then%>
		...
		<a HREF="JavaScript:backToLevel('SID' )" title="<%=strNull%>"><%=Server.HTMLEncode(objNSNET.GetStateName(nStateID))%></a><%
	End If%><img src="<%=strCommonImgFolder%>/transp.gif" width="1" height="8" alt="" border="0"></td>
	</tr><tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("ServAdmin","kHierarchyLevel")%>:</td>
	<td colspan="2" valign="baseline"><%
	...
	Else%>
		...
		<a HREF="JavaScript:backToLevel('HLEVEL')"  title="Выбрать уровень иерархии"><%=GetHLevelName(nLevel)%></a><%
	End If%><img src="<%=strCommonImgFolder%>/transp.gif" width="1" height="8" alt="" border="0">
	</td></tr>

	<tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("Common","kEMName")%>:</td>
	<td colspan="2" valign="baseline"><%
	...
	Else%>
		...
		<a HREF="JavaScript:backToLevel('EMID')"  title="Выбрать управление образования"><%=Server.HTMLEncode(objNSNET.GetEducManagementName(strEMID))%></a><%
	End If%><img src="<%=strCommonImgFolder%>/transp.gif" width="1" height="8" alt="" border="0">
	</td></tr>
	<%If kUseSignatureLogon Then %>
	<tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("Common","kLoginWithSignature")%>:</td>
	<td colspan="2" valign="baseline">
    <%
	If lngLoginState = lngLoginStateLast Then%>
	...
	<%End If %>
	<tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("Common","kUser")%>:</td>
	<td colspan="2" valign="baseline"><%
	If lngLoginState = lngLoginStateLast Then%>
	...
	End If %><img src="<%=strCommonImgFolder%>/transp.gif" width="1" height="8" alt="" border="0"></td></tr>
	<tr>
	<td valign="baseline" width="30%" class="body"><%=obLanguage("Common","kPassword")%>:</td><%
	If lngLoginState = lngLoginStateLast Then%>
	<td valign="baseline">
	...
	Else
		%><td valign="baseline" colspan="2"><img src="<%=strCommonImgFolder%>/transp.gif" width="1" height="8" alt="" border="0" /><%
	End If%>
	</td></tr>
	</table>
</form>
	...
	<form method="GET" action="em_login.asp" name="NoCookie">
