...
Const tb_EM_StatForms = 4
Const tb_EM_AdditionalReports = 5
Const tb_EM_ReportConstructor = 6
Const tb_EM_StatReports = 7
Const tb_EM_QA = 8
...
If GetScreenType() = stSimple Or GetScreenType() = stAjax Or bIsAdminInterface Or bIsEducManager Then
	If bIsAdminInterface Then
		arrTabNames = Array("", obLanguage("MenuFolders","kFSASchool",strFunctionalityType), obLanguage("MenuFolders","kFSAAddresses"), obLanguage("MenuFolders","kFSARefBooks"), obLanguage("MenuFolders","kFSAImport"), _
	...
	ElseIf bIsEducManager Then
		...
		arrFolders(4) = tb_EM_StatForms	'- Hard Reports
		arrFolders(5) = tb_EM_AdditionalReports	'- AdditionalReports
		If Not bIsEMRO Then arrFolders(6) = tb_EM_ReportConstructor	'- ReportConstructor
		If kMODULE_STATREPORTS Then arrFolders(7) = tb_EM_StatReports
		If Module_QA_Available() And Not IsDull(MODULE_QA_URL) Then arrFolders(8) = tb_EM_QA
...
Else
	...
	arrTabNames(tbRooms)		= obLanguage("MenuFolders","kFNRooms")
...
'		arrDefaultFolders(miSetupSchoolMove) = tbMovePoolStaff
	...
	arrDefaultFolders(miLearningApplications)	= tbQA
	arrDefaultFolders(miSchoolResources)		= tbSchoolResources
	arrDefaultFolders(miMySettings)				= tbSettings
...
'			If HasUserRight(arReportsViewForAssignedClass) Then arrFolders(1) = tbReports : Exit Sub
		...
		Case miLearningApplications
			If HasUserRight(arLAViewMaterials) Or HasUserRight(arLAViewSelf) Or HasUserRight(arLAViewAll) Or HasUserRight(arLAEditSelf) Then arrFolders(1) = tbQA
			If HasUserRight(arAddLA) Then arrFolders(2) = tbLAImport
			If HasUserRight(arLASetPolicies) Then arrFolders(3) = tbApplPol
			If HasUserRight(arLACreateGradingScales) And strFunctionalityType<>kFuncType_PreSchool Then arrFolders(4) = tbGrScales
		Case miSchoolResources
			arrFolders(1) = tbSchoolResources
			arrFolders(2) = tbPersonalPortfolios
			arrFolders(3) = tbProjectPortfolios	
		Case miMySettings
			arrFolders(1) = tbSettings
			If MarksBySmsAvailable() Then arrFolders(2) = tbMarksBySms
		Case miAssignments
			If HasUserRight(arAssignmentsViewComplete) Then arrFolders(1) = tbAssignments
			If HasUserRight(arLAViewMaterials) Then arrFolders(2) = tbLearningModules
			If (HasUserRight(arBrowseResultsEGEAllClasses) Or HasUserRight(arBrowseResultsEGEHisClassesOrSubjects) Or HasUserRole(rlParent)_
				Or HasUserRole(rlStudent)) And obTokenMgr.GetData(strToken, "RESULTSEGE") Then arrFolders(3) = tbEgeResults
	End Select
End Sub
