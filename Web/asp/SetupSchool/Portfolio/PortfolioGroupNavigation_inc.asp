...
function movePrev()
{
  ...
  {
	if( getItemLevel(list[nNewPos].value) <= nLevel )
	  break;
  }
...
function moveNext()
{
  ...
  nOldPos = getItemPos(list, <%=strGroupID%>);
  if( nOldPos < 0 || nOldPos == list.length - 1 )
	return;
  ...
  nBranchSize = getBranchSize(list, nOldPos);
  if( nOldPos + nBranchSize == nNewPos )
	return;
...
Sub onDrawPage()
	...
	<FORM NAME="GroupEdit" METHOD="post" onsubmit="return false;" class="form-horizontal">
		<%=WriteObligatoryTags()%>
		<%=WritePortfolioObligatoryTags()%>
		<%=WriteHiddenTags(Array("ACT", strAct, "ParentGRID", "0", "GRNum", "1"))%><%
		...
		<div class="row">
			<div class="col-md-6"><%
				...
				If nItemCount > 1 Then%>
					<h3><%=obLanguage("ResourceGroups","kGroupPosition")%></h3>
					<table class="table table-nobordered">
						<tr>
							<td>
								<SELECT style="overflow-y: auto;" NAME="GroupList" SIZE="<%=nItemCount%>" onChange="restoreSelection()" class="form-control"><%
									...
									While Not objRsGroups.EOF
										...
										If strGroupID = "0" And strID = strParentGroupID Then
											...
											Response.Write obLanguage("ResourceGroups","kNewGroup")
											Response.Write "</OPTION>"
										End If
							...
							<td>
								<table>
									<tr>
										<td>&nbsp;</td>
										<td><%Call ArrowUpButton("movePrev()", obLanguage("ResourceGroups","kShiftUp"))%></td>
										<td>&nbsp;</td>
									</tr>
									<tr>
										<td><%Call ArrowLeftButton("moveUp()", obLanguage("ResourceGroups","kOneLevelUp"))%></td>
										<td>&nbsp;</td>
										<td><%Call ArrowRightButton("moveDown()", obLanguage("ResourceGroups","kOneLevelDown"))%></td>
									</tr>
									<tr>
										<td>&nbsp;</td>
