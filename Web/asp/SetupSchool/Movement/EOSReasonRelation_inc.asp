...
function deleteOldEditSelects()
{
	if( $('select[id=EditingSelect]')[0] )
	{
		...
		if (navigator.userAgent.indexOf ('MSIE') != -1)
		{
			...
			movEOSSpan.innerHTML = '<nobr>' + moveEOSSelect.options[moveEOSSelect.selectedIndex].text + '</nobr>';
			if( moveReasonSelect )
			{
...
function drawMovEOSSelect( td )
{
	...
	if (navigator.userAgent.indexOf ('MSIE') != -1)
	{
		...
		movEOSSelect.attachEvent('onchange', function(){ setHiddenInputValue(movEOSSelect); <%If nDocType = kDocType_OUT  Or nDocType = kDocType_GRADUATE Then%>setReasonType(movEOSSelect,false);<%End If%> });
	...
	else
	{
		...
		movEOSSelect.setAttribute('id','EditingSelect');
		movEOSSelect.setAttribute('studentID',nameOfSelect.replace('EOS_',''));
		movEOSSelect.addEventListener("change", function(){ setHiddenInputValue(this); <%If nDocType = kDocType_OUT  Or nDocType = kDocType_GRADUATE Then%>setReasonType(this,false);<%End If%> }, false);
	}
...
function setHiddenInputValue( select )
{
	...
	{
		var hiddenInput = $('input[type=hidden][id^=REASON_]',select.parentNode)[0];
	...
	else
	{
		...
		{
			hiddenInput.setAttribute('IsOtherState', '');
			hiddenInput.removeAttribute('IsOtherCityInState');
			hiddenInput.removeAttribute('IsInCity');
		}
		else
		{
			if( elem.getAttribute('IsOtherCityInState') != null )
			{
				hiddenInput.setAttribute('IsOtherCityInState', '');
				hiddenInput.removeAttribute('IsOtherState');
				hiddenInput.removeAttribute('IsInCity');
			}
			else
			{
				hiddenInput.setAttribute('IsInCity', '');
				hiddenInput.removeAttribute('IsOtherState');
				hiddenInput.removeAttribute('IsOtherCityInState');
			}
		}
...
function drawOptions(reasonSelect, eo, bfirst)
{
	...
	var bIsOtherState = (elem.getAttribute('isotherstate') != null);
	var bIsOtherCityInState = (elem.getAttribute('isothercityinstate') != null);
	var eotypeid = elem.getAttribute('eotypeid');
	var eoformid = elem.getAttribute('eoformid');

	for (var i = arrReasonOptions.length; --i >= 0;)
		if( arrReasonOptions[i] && isOptionMustBeShow(arrReasonOptions[i], eotypeid, eotypeid, bIsOtherState, bIsOtherCityInState ) )
...
function setReasonType( eo, bFirst )
{
	...
	if( !moveReasonSelect )
	{
		if (navigator.userAgent.indexOf ('MSIE') != -1)
		{
			...
			moveReasonSelect.attachEvent('onchange', function(){ setHiddenInputValue(moveReasonSelect); } );
		}
		else
		{
			moveReasonSelect = document.createElement ('select');
			moveReasonSelect.setAttribute('id','REASON_SELECT_' + studentid);
			moveReasonSelect.addEventListener("change", function(){ setHiddenInputValue(moveReasonSelect);}, false );
		}
	}
...
function setHiddenReasonInput( Reasonselect )
{
	$('input[type=hidden]',Reasonselect.parentNode)[0].value = Reasonselect.value;
}

function testAllSelects()
{
	$('input[id=hidden_MOVEO_SELECT]').each(function(el) { setReasonType(this, true) });
}
-->
</script>
<%End Sub%>