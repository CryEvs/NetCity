...
Dim nDocType, objDocs, FutureYearId, strClassName', strClassID
...
Sub ReadState()
	...
	FutureYearId = GetSafeLng(Request("FutureYearId"), 0)
	strClassName = obTokenMgr.GetData( strToken, "CurrClassName")
End Sub

Sub Main
	Dim currYearID
'	Set objDocs = objNSNET.GetMoveBookDocs(IIF( FutureYearId>0,FutureYearId,strCurrYearID), nDocType, Nothing, strClassName)

	currYearID = IIF( FutureYearId>0,FutureYearId,strCurrYearID)
...
'	Response.Write GetPageTitleExcel(obLanguage("Movement","kTitle_MoveBook",strFunctionalityType), Array(obLanguage("Common","kSchoolYear"), obTokenMgr.GetData(strToken, "CurrYearName"), obLanguage("Movement","kDocType"), GetDocTypeName(nDocType), obLanguage("Common","kClass",strFunctionalityType), strClassName))
	Response.Write GetPageTitleExcel(obLanguage("Movement","kTitle_MoveBook",strFunctionalityType), arrForTitle)
	If objDocs.EOF Then
		Response.Write GetWarningExcel(obLanguage("Movement","kDocsEmpty",strFunctionalityType))
	Else
