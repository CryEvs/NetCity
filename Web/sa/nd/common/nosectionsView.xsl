		...
		<xsl:for-each select="Project/Chapter">
			...
			<xsl:if test="Opened='1'">
						<xsl:for-each select="Lesson">
							...
							<A>
								<xsl:attribute name="HREF">JavaScript:openCurrLsn('<xsl:value-of select="../@ID"/>','0','<xsl:value-of select="@ID"/>','<xsl:apply-templates select="." mode="ls_pimg"/>'); return false;</xsl:attribute> 
								<xsl:attribute name="onclick">JavaScript:openCurrLsn('<xsl:value-of select="../@ID"/>','0','<xsl:value-of select="@ID"/>','<xsl:apply-templates select="." mode="ls_pimg"/>'); return false;</xsl:attribute>
								<IMG>
