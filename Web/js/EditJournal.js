...
function assignsCtrl(lessonName, minMark, maxMark, ha_exists, maxAssignTitleLen, maxAssignNameLen, isDebug){
	...
	var add_buttons = function(){
		...
		return {
			...
			, add_ha : _add_ha
			, add_tkr : _add_tkr
			, hide : function() { _add.hide(); _add_ha.hide(); _add_tkr.hide(); }
			, show : function() {
				if( TKR_mode ) {
	...
	var _add = function(params) {
		...
		if (lesson_name.length == 0)
			if (params && params.isHA)
				row_lessons_ha.show();
			else
				row_lessons.show();
		assign_theme.show();

		if (TKR_mode && params && params.isTKR)
			_counts.tkr++;
		else {
			_counts.other++;
			if (params && params.isHA) {
		...
		compos_assignment = function(params) {
			document.Gradebook.RegimeAddAssign.value = 1;
			document.Gradebook.HomeAssign.value = (params && params.isHA) ? 1 : 0;
			document.Gradebook.TKRAssign.value = (params && params.isTKR) ? 1 : 0;
			...
			var _aply_success = function(response, textStatus) {
				...
				} else if (params && params.isTKR)
					aIdInput.attr("TKR", true);
			}
			var _aply_fail = function(XMLHttpRequest, textStatus, errorThrown) {
				setDBFree();
				bServerCalled = false;
				if (isDebug)
			...
			var _cancel = function() {
				...
				if (params) {
					if (params.isHA) {
						ha_exists = false;
						_counts.other--;
					} else if (params.isTKR) _counts.tkr--;
				} else
					_counts.other--;
			...
			var _reset = function() {
				$("#new_M").remove();
				$("#new_ANTitle").remove();
				dataWereChanged = _dataWereChangedBefore;
				_cancel();
			}
			...
			return {
				aply: function() {
					...
					if (TKR_mode) {
						if (bEmptyMark && !confirm(language.Grade.kConfirmEmptyTKRMarks))
							return false;
					...
					if (moduleQA ) {
						if (!createTestPlanAllowed || ((nAType != 2 && nAType != 4 && nAType != 8 && nAType != 14) || this.isHA)) {
					...
					if (TKR_mode) assigns.tkr_marktypes.disableMarks(form);
				...
				cancel: _cancel,
				isHA: (params && params.isHA == true),
				isTKR: (TKR_mode && params && params.isTKR == true)
			}
		}(params);
		_setDefaultName();
	};
	...
	var _delete = function(obj) {
		...
		var sID = obj.id;
		var arrVals = sID.split('_');
		if (arrVals.length != 3){
			alert(language.Generic.Common.kUnexpErr);
			return;
		}
		var sAID = arrVals[1];
		var $checkM = $("input:checkbox[name=M_" + sAID + "][checked=true]", form);
		var nLenM = $checkM.length;
		var nLenG = 0;
		if (nLenM == 0){
			var $checkG = $("input:text[name='G_" + sAID + "'][value!=''][value!=' ']", form);
			nLenG = $checkG.length;
		}

		if ((nLenM + nLenG) == 0){
			if( !confirm(language.Generic.Assignment.kSureToDeleteAssignment ) )
				return;
		}
		else if( !confirm(language.Generic.Grade.kDelAssignmentWithMarks ) )
			return;

		if( isDBBusy() ) return false;
		var form = document.EditAssign;

		_delete.AssignID = sAID;
		_delete.IsHA = (arrVals[2] == "1");
		form.ADEL.value = sAID;
		jsSubmit({
			form: form,
	...
	var _del_Success = function (response, textStatus) // data - object, textStatus - string
	{
		setDBFree();
		bServerCalled = false;
		...
		var delAssignInput = $("input:hidden[name=AID][value=" + _delete.AssignID + "]", form);
		if( typeof(delAssignInput.attr("TKR")) != "undefined" ) assigns.counts.tkr--; else assigns.counts.other--;
		$("#td_AssignTitle_" + _delete.AssignID).remove();
		$("#td_MarksColumn_" + _delete.AssignID).remove();
		...
		_delete.AssignID = null;
		_delete.IsHA = false;
	}

	var _del_Fail = function (XMLHttpRequest, textStatus, errorThrown)
	{
		setDBFree();
		bServerCalled = false;
		alert(language.Generic.Common.kUnexpErr + "\n" + (isDebug ? XMLHttpRequest.responseText : ''));
		_delete.AssignID = null;
		_delete.IsHA = false;
	}
